...
device = "cuda"
model_name = "rozariwang/GPT-BPE-sfs"
...
# Function to generate molecules and calculate log-likelihood
def generate_molecules(model, tokenizer, num_molecules, max_length, temperature, filename, batch_size=32):
    model.eval()
    eos_token_id = tokenizer.vocab.get('</s>')  # End of Sequence Token
    bos_token_id = tokenizer.vocab.get('<s>')  # Start of Sequence Token
    ...
    with open(filename, mode='a', newline='') as file:
        ...
        with torch.no_grad():
            for _ in range(0, num_molecules, batch_size):
                ...
                # Initialize a binary mask to track active sequences (sequences that haven't reached </s>)
                ...
                for _ in range(max_length):
                    ...
                    # Mask out inactive sequences by forcing them to generate </s> repeatedly
                    ...
                    # Update the active mask for sequences that have reached </s>
                ...
                for i in range(current_batch_size):
                    ...
                    if len(molecules) >= batch_size:
                        # Attempt to save the batch and skip entries that cause errors
                        try:
                            writer.writerows(zip(molecules, loglikelihoods))
                        except csv.Error as e:
                            print(f"Skipping a problematic molecule due to error: {e}")
                            continue  # Skip the problematic entry and move on
...
temperature = 1
file_name = 'GPT_bpe_sfs_generated_mol_100000.csv'
