...
device = "cuda"
model_name = "rozariwang/M1-Char-sfs"
vocab_file = "./hhwang/shiawaseda/Datasets/vocab.json"
tokenizer = CharLevelTokenizer(vocab_file)
...
def generate_molecules(model, tokenizer, num_molecules, max_length, temperature, filename, batch_size=32):
    model.eval()
    eos_token_id = tokenizer.vocab.get('[SEP]')  # End of Sequence Token
    bos_token_id = tokenizer.vocab.get('[CLS]')  # Start of Sequence Token
        ...
        with tqdm(total=num_molecules, desc="Generating molecules", unit="molecule") as pbar:
            with torch.no_grad():
                for _ in range(0, num_molecules, batch_size):
                    ...
                    eos_token_id = tokenizer.vocab.get('[SEP]')  # Use only the EOS token as the termination criterion
...
temperature = 1
file_name = 'M1_Char_sfs_generated_mol_100000.csv'
