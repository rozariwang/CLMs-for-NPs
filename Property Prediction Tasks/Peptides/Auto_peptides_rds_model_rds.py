...
model_names = [
    "rozariwang/GPT-Char-rds",
    "rozariwang/M2-BPE-rds", "rozariwang/M1-BPE-rds", "rozariwang/GPT-BPE-rds",
    "rozariwang/M2-AIS-rds", "rozariwang/M1-AIS-rds", "rozariwang/GPT-AIS-rds",
    "rozariwang/M2-npbpe60-rds", "rozariwang/M1-npbpe60-rds", "rozariwang/GPT-npbpe60-rds",
    "rozariwang/M2-npbpe100-rds", "rozariwang/M1-npbpe100-rds", "rozariwang/GPT-npbpe100-rds",
    "rozariwang/M2-npbpe1000-rds", "rozariwang/M1-npbpe1000-rds", "rozariwang/GPT-npbpe1000-rds",
    "rozariwang/M2-npbpe7924-rds", "rozariwang/M1-npbpe7924-rds", "rozariwang/GPT-npbpe7924-rds",
    "rozariwang/M2-npbpe30k-rds", "rozariwang/M1-npbpe30k-rds", "rozariwang/GPT-npbpe30k-rds"
]

#"rozariwang/M2-Char-rds", "rozariwang/M1-Char-rds", 
...
def crossval_hyperparameter_search(model_name, tokenizer, num_folds=5):
    ...
    for lr, batch_size in product(learning_rates, batch_sizes):
        ...
        for fold in range(1, num_folds + 1):
            train_df = pd.read_csv(f'./hhwang/shiawaseda/Downstream_Data/Peptides_CV/peptides_fold{fold}_rd_train.csv')
            val_df = pd.read_csv(f'./hhwang/shiawaseda/Downstream_Data/Peptides_CV/peptides_fold{fold}_rd_val.csv')
            
            train_dataset = PeptidesDataset(train_df, tokenizer, target_column)
            val_dataset = PeptidesDataset(val_df, tokenizer, target_column)
            ...
            model = MambaForSequenceClassificationFull(model_name, 1) if 'M' in model_name else GPT2ForSequenceClassification(model_name, 1)
        ...
        if avg_auc > best_val_auc:
            best_val_auc, best_config = avg_auc, {'learning_rate': lr, 'batch_size': batch_size}
...
def train_and_evaluate(model_name, tokenizer, best_config, fold, run):
    ...
    train_df = pd.read_csv(f'./hhwang/shiawaseda/Downstream_Data/Peptides_CV/peptides_fold{fold}_rd_train.csv')
    val_df = pd.read_csv(f'./hhwang/shiawaseda/Downstream_Data/Peptides_CV/peptides_fold{fold}_rd_val.csv')
    test_df = pd.read_csv(f'./hhwang/shiawaseda/Downstream_Data/Peptides_CV/peptides_fold{fold}_rd_test.csv')
    target_column = 'Permeability_Label'
    
    train_dataset = PeptidesDataset(train_df, tokenizer, target_column)
    val_dataset = PeptidesDataset(val_df, tokenizer, target_column)
    test_dataset = PeptidesDataset(test_df, tokenizer, target_column)
    ...
    model = GPT2ForSequenceClassification(model_name, num_labels=1) if 'GPT' in model_name else MambaForSequenceClassificationFull(model_name, 1)
    ...
    print(f"Model: {model_name}, Fold: {fold}, AUC-ROC: {auc:.4f}")
    return auc
...
def cross_validate(model_name, tokenizer, best_config):
    ...
    for fold in range(1, 6):
        ...
        for run in range(1, 6):
            auc = train_and_evaluate(model_name, tokenizer, best_config, fold, run)
            run_auc.append(auc)
        fold_mean_auc = np.mean(run_auc)
        fold_auc.append(fold_mean_auc)

    overall_mean_auc = np.mean(fold_auc)
    overall_std_auc = np.std(fold_auc)
    overall_se_auc = overall_std_auc / np.sqrt(len(fold_auc))
...
for model_name in model_names:
    ...
    best_val_auc, best_config = crossval_hyperparameter_search(model_name, tokenizer)
