...
def crossval_hyperparameter_search(model_name, tokenizer, num_folds=5):
    ...
    for lr, batch_size in product(learning_rates, batch_sizes):
        ...
        fold_mccs = []
        target_column = 'Taste_Label'

        for fold in range(1, num_folds + 1):
            train_df = pd.read_csv(f'./hhwang/shiawaseda/Downstream_Data/4Tastes_CV/4Tastes_fold{fold}_sf_train.csv')
            val_df = pd.read_csv(f'./hhwang/shiawaseda/Downstream_Data/4Tastes_CV/4Tastes_fold{fold}_sf_val.csv')
            
            train_dataset = TastesDataset(train_df, tokenizer, target_column)
            val_dataset = TastesDataset(val_df, tokenizer, target_column)
            ...
            model = MambaForSequenceClassificationFull(model_name, 4, class_weights) if 'M' in model_name else GPT2ForSequenceClassification(model_name, 4, class_weights)
...
def train_and_evaluate(model_name, tokenizer, best_config, fold, run):
    ...
    train_df = pd.read_csv(f'./hhwang/shiawaseda/Downstream_Data/4Tastes_CV/4Tastes_fold{fold}_sf_train.csv')
    val_df = pd.read_csv(f'./hhwang/shiawaseda/Downstream_Data/4Tastes_CV/4Tastes_fold{fold}_sf_val.csv')
    test_df = pd.read_csv(f'./hhwang/shiawaseda/Downstream_Data/4Tastes_CV/4Tastes_fold{fold}_sf_test.csv')
    target_column = 'Taste_Label'
    
    train_dataset = TastesDataset(train_df, tokenizer, target_column)
    val_dataset = TastesDataset(val_df, tokenizer, target_column)
    test_dataset = TastesDataset(test_df, tokenizer, target_column)
    ...
    model = GPT2ForSequenceClassification(model_name, num_labels=4, class_weights=class_weights) if 'GPT' in model_name else MambaForSequenceClassificationFull(model_name, 4, class_weights)
