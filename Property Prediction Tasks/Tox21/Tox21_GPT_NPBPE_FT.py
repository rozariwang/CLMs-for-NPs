...
tokenizer_path = './hhwang/shiawaseda/Datasets/npbpe_tokenizer.json'
tokenizer = NPBPETokenizer(tokenizer_path)
base_model = "rozariwang/GPT-npbpe30k-rds"
...
best_hyperparams = {
    "NR-AR": {"learning_rate": 5e-05, "batch_size": 8},
    "NR-AR-LBD": {"learning_rate": 0.0001, "batch_size": 8},
    "NR-AhR": {"learning_rate": 0.0001, "batch_size": 16},
    "NR-Aromatase": {"learning_rate": 5e-05, "batch_size": 8},
    "NR-ER": {"learning_rate": 5e-05, "batch_size": 8},
    "NR-ER-LBD": {"learning_rate": 0.0001, "batch_size": 16},
    "NR-PPAR-gamma": {"learning_rate": 0.0001, "batch_size": 16},
    "SR-ARE": {"learning_rate": 0.0001, "batch_size": 16},
    "SR-ATAD5": {"learning_rate": 5e-05, "batch_size": 8},
    "SR-HSE": {"learning_rate": 0.0001, "batch_size": 8},
    "SR-MMP": {"learning_rate": 0.0001, "batch_size": 16},
...
def collate_batch(batch):
    ...
    batch_padded_inputs = pad_sequence(input_ids, batch_first=True, padding_value=tokenizer.tokenizer.token_to_id('[PAD]')).to(device)
    attention_mask = (batch_padded_inputs != tokenizer.tokenizer.token_to_id('[PAD]')).float().to(device)
...
def cross_validate(subtask):
    ...
    print(f"\n{subtask}:")
    print(f"Mean AUC-ROC: {overall_mean_auc:.4f}, Std: {overall_std_auc:.4f}, SE: {overall_se_auc:.4f}")
