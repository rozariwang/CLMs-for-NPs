...
def train(model, train_loader, val_loader, optimizer, criterion, device, num_epochs, patience=5):
    ...
    for epoch in range(num_epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), "gpt_npbpe30k_sfs_best_model.pth")  # Save the best model weights
        else:
            patience_counter += 1
        ...
        if patience_counter >= patience:
            ...
            model.load_state_dict(torch.load("gpt_npbpe30k_sfs_best_model.pth"))  # Load best model
...
tokenizer = NPBPETokenizer("./hhwang/shiawaseda/Datasets/npbpe_tokenizer.json")
...
optimizer = SAM(model.parameters(), base_optimizer, lr=0.001, rho=0.05, weight_decay=0.0001)
criterion = CrossEntropyLoss(ignore_index=0)
   

num_epochs = 150

wandb.init(project="gpt-npbpe30k-sfs-training", config={
    "learning_rate": 0.001,
    "epochs": num_epochs,
...
save_directory = './gpt_npbpe30k_scaffold_split'
...
try:
    ...
    artifact = wandb.Artifact('gpt-npbpe30k-scaffold-split', type='model')
