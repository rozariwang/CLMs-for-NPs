...
def train(model, train_loader, val_loader, optimizer, criterion, device, num_epochs, patience=5):
    ...
    for epoch in range(num_epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), "M1_npbpe7924_sfs_bestmodel.pth")  # Save the best model weights
        else:
            patience_counter += 1
        ...
        if patience_counter >= patience:
            ...
            model.load_state_dict(torch.load("M1_npbpe7924_sfs_bestmodel.pth"))  # Load best model
...
config = MambaConfig(
    d_model=256,
    n_layer=4,
    d_intermediate=1024, #has to be d_model*4
    vocab_size=7924,
...
wandb.init(project="mamba-1-npbpe7924-sfs-training", config={
...
save_directory = './Mamba1_npbpe7924_scaffold_split'
...
try:
    ...
    artifact = wandb.Artifact('mamba1-npbpe7924-scaffold-split', type='model')
