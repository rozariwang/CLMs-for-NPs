...
def train(model, train_loader, val_loader, optimizer, criterion, device, num_epochs, patience=5):
    ...
    for epoch in range(num_epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), "gpt_bpe_sfs_best_model.pth")  # Save the best model weights
        else:
            patience_counter += 1
        ...
        if patience_counter >= patience:
            ...
            model.load_state_dict(torch.load("gpt_bpe_sfs_best_model.pth"))  # Load best model
...
config = GPT2Config(
    ...
    n_inner=2048,       # MLP dimension (feed-forward hidden size) 
    bos_token_id=0,  # ID for the beginning-of-sequence token
    eos_token_id=2   # ID for the end-of-sequence token
)
...
wandb.init(project="gpt2-bpe-sfs-training", config={
...
save_directory = './gpt_bpe_scaffold_split'
...
try:
    ...
    artifact = wandb.Artifact('gpt-bpe-scaffold-split', type='model')
