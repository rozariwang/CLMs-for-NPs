...
def train(model, train_loader, val_loader, optimizer, criterion, device, num_epochs, patience=5):
    ...
    for epoch in range(num_epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), "M1_Char_sfs_best_model.pth")  # Save the best model weights
        else:
            patience_counter += 1
        ...
        if patience_counter >= patience:
            ...
            model.load_state_dict(torch.load("M1_Char_sfs_best_model.pth"))  # Load best model
...
wandb.init(project="mamba-1-char-sfs-training", config={
...
save_directory = './Mamba1_char_scaffold_split'
...
try:
    ...
    artifact = wandb.Artifact('mamba1-char-scaffold-split', type='model')
