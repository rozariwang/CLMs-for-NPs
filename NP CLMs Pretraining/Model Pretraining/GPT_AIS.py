...
class SMILESDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            print(f"Error encoding SMILES: {smiles_string}. Skipping. Error: {e}")
            return None  # Indicate invalid data point
...
def collate_batch(batch):
    ...
    if len(batch) == 0:
        return None, None  # Handle invalid batches 
    ...
    for epoch in range(num_epochs):
        ...
        with tqdm(total=len(train_loader), desc=f"Epoch {epoch + 1}/{num_epochs}", unit="batch") as pbar:
            for batch_idx, (inputs, targets) in enumerate(train_loader):
                if inputs is None or targets is None:  # Skip invalid batches
                    continue
                ...
                # Log training loss every 10 batches (optional, for more frequent updates)
                if batch_idx % 10 == 0:
        ...
        if avg_val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), "gpt_ais_sfs_best_model.pth")  # Save the best model weights
        else:
            patience_counter += 1
        ...
        if patience_counter >= patience:
            ...
            model.load_state_dict(torch.load("gpt_ais_sfs_best_model.pth"))  # Load best model
...
tokenizer = AISTokenizer(vocab_file="./hhwang/shiawaseda/Datasets/ais_vocab.json")
...
# Define configuration for a smaller custom GPT model
config = GPT2Config(
    ...
    n_inner=2048,       # MLP dimension (feed-forward hidden size) (usually 4 * n_embd??)
    bos_token_id=2,  # ID for the beginning-of-sequence token
    eos_token_id=3   # ID for the end-of-sequence token
)
...
optimizer = SAM(model.parameters(), base_optimizer, lr=0.0001, rho=0.05, weight_decay=0.0001)
criterion = CrossEntropyLoss(ignore_index=0)
   
num_epochs = 150

wandb.init(project="gpt-ais-sfs-training", config={
...
def save_pretrained(model, tokenizer, save_directory):
    ...
    # Save the tokenizer's vocab and configuration
...
save_directory = './gpt_ais_scaffold_split'
...
try:
    ...
    artifact = wandb.Artifact('gpt-ais-scaffold-split', type='model')
