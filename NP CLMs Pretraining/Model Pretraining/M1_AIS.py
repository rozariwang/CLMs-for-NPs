...
def collate_batch(batch):
    ...
    targets = batch_padded[:, 1:]  # Shifted version for the targets
    inputs = inputs.long()
...
def evaluate(model, data_loader, criterion, device):
    ...
    with torch.no_grad():
        for _, (inputs, targets) in enumerate(data_loader):
            ...
            # outputs is an instance of CausalLMOutput, access logits
...
def train(model, train_loader, val_loader, optimizer, criterion, device, num_epochs, patience=5):
    ...
    for epoch in range(num_epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), "M1_ais_sfs_best_model.pth")  # Save the best model weights
        else:
            patience_counter += 1
        ...
        if patience_counter >= patience:
            ...
            model.load_state_dict(torch.load("M1_ais_sfs_best_model.pth"))  # Load best model
...
config = MambaConfig(
    d_model=512,
    n_layer=8,
    d_intermediate=2048, #has to be d_model*4
    vocab_size=1023,
...
wandb.init(project="mamba-1-ais-sfs-training", config={
...
save_directory = './Mamba1_ais_scaffold_split'
...
try:
    ...
    artifact = wandb.Artifact('mamba1-ais-scaffold-split', type='model')
