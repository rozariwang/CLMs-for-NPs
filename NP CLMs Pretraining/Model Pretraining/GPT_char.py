...
def train(model, train_loader, val_loader, optimizer, criterion, device, num_epochs, patience=5):
    ...
    for epoch in range(num_epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), "gpt_char_sfs_best_model.pth")  # Save the best model weights
        else:
            patience_counter += 1
        ...
        if patience_counter >= patience:
            ...
            model.load_state_dict(torch.load("gpt_char_sfs_best_model.pth"))  # Load best model
...
wandb.init(project="gpt-char-sfs-training", config={
...
def save_model_as_safetensors(model, save_directory, filename='model.safetensors'):
    ...
    with open(os.path.join(save_directory, filename), 'wb') as f:
        f.write(struct.pack('<Q', metadata_length))  # Write the length of metadata as an unsigned long
...
save_directory = './gpt_char_scaffold_split'
...
try:
    ...
    artifact = wandb.Artifact('gpt-char-scaffold-split', type='model')
