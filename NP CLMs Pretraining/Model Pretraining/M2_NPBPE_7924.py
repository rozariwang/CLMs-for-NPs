...
def train(model, train_loader, val_loader, optimizer, criterion, device, num_epochs, patience=5):
    ...
    for epoch in range(num_epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), "M2_npbpe7924_sfs_best_model.pth")  # Save the best model weights
        else:
            patience_counter += 1
        ...
        if patience_counter >= patience:
            ...
            model.load_state_dict(torch.load("M2_npbpe7924_sfs_best_model.pth"))  # Load best model
...
tokenizer = NPBPETokenizer("./hhwang/shiawaseda/Datasets/npbpe_7924vocab.json")
...
wandb.init(project="mamba-2-npbpe7924-sfs-training", config={
...
def save_pretrained(model, tokenizer, save_directory):
    ...
    # Check if tokenizer has get_vocab() or other method to obtain vocab
...
def save_model_as_safetensors(model, save_directory, filename='model.safetensors'):
    ...
    with open(os.path.join(save_directory, filename), 'wb') as f:
        f.write(struct.pack('<Q', metadata_length))  
...
save_directory = './Mamba2_npbpe7924_scaffold_split'
...
try:
    ...
    artifact = wandb.Artifact('mamba2-npbpe7924-scaffold-split', type='model')
