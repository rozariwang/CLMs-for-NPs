...
def collate_batch(batch):
    ...
    inputs = batch_padded[:, :-1] # select all rows (data points), but skip last item in the column, shifts the sequence one token to the left
    targets = batch_padded[:, 1:]  # skip first, shifts the sequence one token to the right
    inputs = inputs.long()
...
for _ in range(num_search_iters):
    ...
    config = GPT2Config(
        ...
        n_embd=n_embd, # Embedding size
        n_layer=n_layer, # Number of Transformer layers
        n_head=n_head,  # Number of attention heads
        n_inner=n_embd * 4, # MLP dimension (feed-forward hidden size) 
...
print("\nBest hyperparameter set for GPT_Char_sf:")
